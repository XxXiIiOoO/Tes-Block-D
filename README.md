...
Эти аккаунты сейчас заданы в корневом `.env` и создаются автоматически при старте backend. Все они помечаются как подтвержденные.

| Роль | Email | Username | Пароль | Доступ |
| --- | --- | --- | --- | --- |
| Admin | `xacha123456@gmail.com` | `xacha_admin` | `A910kda910kd` | полный доступ ко всем проектам, пользователям, аудитам и настройкам ролей |
| Worker | `yrnado@mail.ru` | `yrnado_worker` | `A910kda910kd` | создание проектов, тестов, запусков в своем контуре |
| Viewer | `viewer@blocktest.local` | `viewer_user` | `A910kda910kd` | только просмотр, без создания и запуска |

Важно: эти данные подходят для локального стенда и демонстрации. Если README будет опубликован, пароли нужно заменить.
